...
use App\Http\Requests\ThirdPartyInterfaces\V2\ModifyBeanRequest;
use App\User;
...
class ModifyBeanInterfaceController extends Controller
{
    ...
    /**
     * @api            {post} /v2/modify-bean 修改用户迈豆
     * @apiName        modify-bean
     * @apiDescription 为用户修改迈豆，计为空中课堂推广行为发放的迈豆。
     ...
     *     curl -X "POST" "https://med-union-user-plateform.dev/api/v2/modify-bean" \
    ...
    {
        $user->modifyBeanAccordingToBeanRate(BeanRate::where('name_en', 'air_classes_popularize')->firstOrFail(), $bean);
    }
}
